...
public class ExpressionParserExample1 {
	public static void main(String[] args) {
		...
		Expression expression = expressionParser.parseExpression("'Good Morning'");
		String strVal = expression.getValue(String.class);
		System.out.println("1. Literal expression value:\n" + strVal);

		// 2. Method invocation
		expression = expressionParser.parseExpression("'Vishalakshi'.concat('..,')");
		...
		expression = expressionParser.parseExpression("'1235' matches '[0-9\\s]+'");
		boolVal = expression.getValue(Boolean.class);
